...
class _VehicleScheduleScreenState
    extends State<VehicleScheduleScreen> {
  ...
  String? _validateSlotName(String? value) {
    if (value == null || value.trim().isEmpty) return "Slot name required";
    final regex = RegExp(r'^\d{1,2}-\d{1,2}\s+(AM|PM)$');
    if (!regex.hasMatch(value.trim())) {
      return "Format must be like  8-10 AM  or  12-2 PM";
    }
    if (slots.contains(value.trim())) return "This slot already exists";
    return null;
  }
      ...
      builder: (context) => AlertDialog(
        ...
        content: Form(
          ...
          child: Column(
            ...
            children: [
              const Text("Format examples:",
                  style: TextStyle(fontSize: 12, color: Colors.black54)),
              const Text("  8-10 AM    12-2 PM    4-6 PM    9-11 PM",
                  style: TextStyle(
                      fontSize: 12,
                      color: Colors.teal,
  ...
  Future<void> _saveSchedule() async {
    ...
    bool success =
    await VehicleScheduleApiService.saveSchedule(data);
